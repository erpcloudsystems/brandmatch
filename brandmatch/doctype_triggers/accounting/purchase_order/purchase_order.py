...
def item_query(doctype, txt, searchfield, start, page_len, filters, as_dict=False):
    item_names = frappe.db.get_all("Item Supplier", {"supplier": filters.get("supplier", "")}, ["parent"])
    item_names = [d.parent for d in item_names]
    
    if txt in item_names:
        items = frappe.db.get_all("Item", {"name": txt}, ["name", "description"])  # Get only the item with the item_code entered by the user
    else:
